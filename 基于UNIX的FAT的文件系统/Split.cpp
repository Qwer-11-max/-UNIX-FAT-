...
#include<string.h>
using namespace std;
void Split(WholeName* wholename, char* wholefilename)//需求参数为结构体指针，以及存放整个文件名的字符数组指针
{
	...
	length = strlen(wholefilename);//检测输入的字符数组长度；                 !!!!!!!!!!!!!!wholefilename[WHOLEFILENAMESIZE]一定要初始化再赋值不然此行会出错！！！！！！！！！！！！
