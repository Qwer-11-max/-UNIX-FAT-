...
void creatFile(superBlk* supblk, Files* fls, type_t type, uid_t uid, gid_t gid,FILE* disk);//创建文件
...
void OpenFile(Files* fls, FILE* disk, char* filename, type_t type, Openqueue* queue);//找到文件并且列入打开队列。
void CloseFile(char* filename,type_t type);//从打开队列清除文件
#endif
